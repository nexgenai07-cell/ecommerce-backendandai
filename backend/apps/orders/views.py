...
from .serializers import (
    ...
    CheckoutSerializer, AdminOrderStatusSerializer,
)
...
class CheckoutView(APIView):
    ...
    Converts the user's cart into an order. Stock is re-checked and reduced
    inside an atomic transaction to prevent race conditions (two people
    buying the last item at the same time).
    """
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            # Lock the products being purchased so concurrent checkouts can't oversell
            cart_items = list(cart.items.select_related('product').select_for_update().all())

            out_of_stock = []
            for item in cart_items:
                # re-fetch product with lock to get the true current stock
            ...
            customer = get_or_create_customer(request.user, store_id=cart.store_id)
            ...
            for item in cart_items:
                # snapshot product name + price — future price changes won't affect old orders
        ...
        # TODO (BE2): trigger Celery task here -> send_order_confirmation_whatsapp.delay(order.id)
...
class AdminOrderStatusUpdateView(APIView):
    ...
    def put(self, request, order_number):
        ...
        # restore stock if changing TO cancelled from a non-cancelled state
        ...
        if 'tracking_number' in serializer.validated_data and serializer.validated_data['tracking_number']:
            order.tracking_number = serializer.validated_data['tracking_number']
        order.save()

        # TODO (BE2): trigger Celery task -> send_order_status_whatsapp.delay(order.id)
...
class AdminOrderFilterView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if params.get('customer_name'):
            qs = qs.filter(customer__name__icontains=params['customer_name'])
